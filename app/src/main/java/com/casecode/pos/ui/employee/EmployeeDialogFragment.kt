...
@AndroidEntryPoint
class EmployeeDialogFragment : DialogFragment() {
    companion object {
        const val ADD_EMPLOYEE_TAG = "EmployeesDialogFragment"
        const val UPDATE_EMPLOYEE_TAG = "UpdateEmployeeDialogFragment"
        fun newInstance(): EmployeeDialogFragment {
    ...
    private fun validatePasswordEmployeeInput() {
        binding.etEmployeePassword.doAfterTextChangedListener { passwordEditText ->
            ...
            if (TextUtils.isEmpty(passwordEditText)) {
                ...
                binding.tilEmployeePassword.error = getString(R.string.add_employee_password_empty)
            } else if (passwordEditText.toString().length < 6) {
                binding.tilEmployeePassword.error = getString(R.string.add_employee_password_error)
            } else {
