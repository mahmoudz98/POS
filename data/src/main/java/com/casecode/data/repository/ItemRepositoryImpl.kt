...
class ItemRepositoryImpl @Inject constructor(
     ...
     */
    override suspend fun getItems(uid: String): Resource<List<Item>> {
        return withContext(ioDispatcher) {
            try {
                val itemMutableList = mutableListOf<Item>()

                // Query Firestore to retrieve all items for the given UID
                val querySnapshot = getItemCollectionRef(uid).get().await()
                ...
                if (itemMutableList.isEmpty()) {
                    ...
                    // Return the list of items as a success resource
                    Resource.Success(itemMutableList)
                }
            } catch (e: Exception) {
                // Return an error resource if any exception occurs
                Resource.error("Error getting items")
            }
        }
    }
        ...
        return withContext(ioDispatcher) {
            try {
                suspendCoroutine { continuation ->
                    ...
                    val uid = "nRoH5pcgCsbMIHVa6tmN3wkEPFL2"
                    val sku = item.sku
                    getItemDocumentRef(uid = uid, sku = sku).set(itemMap as Map<*, *>)
        ...
        return withContext(ioDispatcher) {
            try {
                suspendCoroutine { continuation ->
                    val uid = "nRoH5pcgCsbMIHVa6tmN3wkEPFL2"
                    val sku = item.sku
                    getItemDocumentRef(uid = uid, sku = sku).delete()
                        .addOnSuccessListener {
     ...
     */
    private fun getItemDocumentRef(uid: String, sku: String): DocumentReference {
        val itemsCollection = firestore.collection(USERS_COLLECTION_PATH)
            .document(uid)
