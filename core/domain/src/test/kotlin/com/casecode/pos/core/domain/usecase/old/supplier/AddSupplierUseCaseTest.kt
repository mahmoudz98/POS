...
class AddSupplierUseCaseTest {
    ...
    private val addSupplierUseCase = AddSupplierUseCase(testSupplierRepository)
    ...
    @Test
    fun whenSupplierAdded_returnsSuccess() = runTest {
        val result = addSupplierUseCase(newSupplier)
    ...
    fun whenHasError_returnsError() = runTest {
        ...
        val result = addSupplierUseCase(newSupplier)
        assertEquals(
            OperationResult.Failure(stringData.core_data_add_supplier_failure_generic),
            result,
        )
    }
}