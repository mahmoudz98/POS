        ...
        try {
            ...
            auth.checkUserNotFound<List<PrinterInfo>> {
                ...
                suspendCoroutine { continuation ->
                    firestore.getCollectionRefFromUser(uid, PRINTER_INFO_COLLECTION_PATH).get()
                        .addOnSuccessListener { documents ->
                            val printerMutableList = mutableListOf<PrinterInfo>()
                            documents.documents.mapNotNull { document ->
                                document.toObject(PrinterInfo::class.java)
        ...
        try {
            ...
            auth.checkUserNotFound<Int> {
                ...
                suspendCoroutine { continuation ->
                    ...
                    firestore.getCollectionRefFromUser(uid, PRINTER_INFO_COLLECTION_PATH)
                        .document(printerInfo.name).set(printerInfo.asExternalMapper())
