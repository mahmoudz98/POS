 ...
 */
class PosPreferencesDataSource @Inject constructor(private val loginPreferences: DataStore<LoginPreferences>) {
    ...
    suspend fun setLoginWithAdmin(uid: String, isCompleteSetupBusiness: Boolean) {
        try {
            loginPreferences.updateData {
                it.copy {
                    authState = if (isCompleteSetupBusiness) {
                        AuthState.LOGIN_ADMIN
                    } else {
                        AuthState.NOT_COMPLETE_SETUP_BUSINESS
                    }
    ...
    suspend fun restLogin() {
        try {
            loginPreferences.updateData {
                it.toBuilder().clearUid().clearEmployee().clearAuthState().build()
