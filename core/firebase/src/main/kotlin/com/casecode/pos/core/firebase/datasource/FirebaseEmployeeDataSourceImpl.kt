...
class FirebaseEmployeeDataSourceImpl @Inject constructor(
    ...
    override suspend fun findBusinessByCompanyCode(companyCode: String): NetworkBusiness? {
        return db.collection(BUSINESSES_COLLECTION_PATH)
            .whereEqualTo(BUSINESS_COMPANY_CODE_FIELD, companyCode.uppercase()).limit(1).get().await()
            .documents.firstOrNull()?.toObject(NetworkBusiness::class.java)
    }

    override suspend fun findEmployeeByIdentifier(businessId: String, employeeIdentifier: String): NetworkEmployee? {
        return db.collection(BUSINESSES_COLLECTION_PATH).document(businessId)
            .collection(EMPLOYEES_FIELD) // Your constant for the 'employees' subcollection
            .whereEqualTo("employeeId", employeeIdentifier).limit(1).get().await()
